...
class Agent(BaseAgent):
    ...
    def _validate_and_correct_action(
        ...
        # ---- 2.5 TYPE 后立即 SCROLL 拦截（仅当 TYPE 后已有 CLICK 时） ----
        if action == ACTION_SCROLL and self._action_history:
            last = self._action_history[-1]
            if last["action"] == ACTION_CLICK and len(self._action_history) >= 2:
                prev = self._action_history[-2]
                if prev["action"] == ACTION_TYPE and last.get("page_changed", True):
                    # TYPE → CLICK（页面变化，说明点了提交按钮）→ SCROLL 多余
                    logger.info(
                        "[Agent] TYPE→CLICK 后 SCROLL，拦截并改为 COMPLETE"
                        "（已输入文字并点击按钮，不应继续滚动）"
                    )
        ...
        # ---- 5. 后置 COMPLETE 检测（关键操作完成后自动判定） ----
        action, parameters = self._check_task_completion(action, parameters)
    ...
    def _check_task_completion(
        ...
        """
        后置任务完成检测：当检测到关键操作已完成时，自动将动作替换为 COMPLETE

        触发条件：
        - 最后一次 TYPE 之后有 CLICK 且页面变化，且之后又有 SCROLL/CLICK 操作 → 多余
        - 只追踪最后一次 TYPE（避免搜索+评论多步输入场景被误杀）
        """
        if action == ACTION_COMPLETE or not self._action_history:
            return action, parameters
        ...
        for i in range(len(history) - 1, -1, -1):
            if history[i]["action"] == ACTION_TYPE:
                last_type_idx = i
                break

        if last_type_idx < 0:
            return action, parameters  # 从未 TYPE 过

        after_last_type = history[last_type_idx + 1:]

        # 检查最后一次 TYPE 之后是否有点击且页面变化（说明提交操作已执行）
        if not after_last_type:
            return action, parameters  # TYPE 是最后一步，还没来得及点按钮

        has_clicked_with_change = False
        for step in after_last_type:
            if step["action"] == ACTION_CLICK and step.get("page_changed", True):
                has_clicked_with_change = True
                break

        if not has_clicked_with_change:
            return action, parameters  # TYPE 后还没成功点击任何按钮

        # 最后一次 TYPE 之后有成功的 CLICK，后续再有 SCROLL → 多余
        # 注意：只拦截 SCROLL，不拦截 CLICK
        # 原因：搜索场景 TYPE→CLICK(搜索按钮)→CLICK(搜索结果) 是正常的
        #        但评论场景 TYPE→CLICK(发送)→SCROLL 是多余的
        if action == ACTION_SCROLL:
            # 排除：after_last_type 中有 TYPE（说明还有后续输入步骤）
            has_subsequent_type = any(s["action"] == ACTION_TYPE for s in after_last_type)
            if not has_subsequent_type:
                logger.info(
                    f"[Agent] 检测到 TYPE + 后续 CLICK 已执行，"
                    f"当前操作 SCROLL 多余，自动改为 COMPLETE"
                )
                return ACTION_COMPLETE, {}
    ...
    def _clamp_click_coordinates(self, x: int, y: int) -> Tuple[int, int]:
        """
        坐标范围校验 + 安全边界内缩 + 边缘坐标向中心修正

        策略：
        - 超出 [0, 1000] 的值裁剪到边界
        - 边界附近（<5 或 >995）内缩 15 个单位
        - 极端边缘（<30 或 >970）额外向中心靠拢 20%，因为 VLM 对边缘区域的定位偏差更大
        """
        x = max(0, min(1000, x))
        y = max(0, min(1000, y))
        ...
        # 边缘坐标向中心修正（VLM 对屏幕边缘定位偏差大）
        CENTER_X, CENTER_Y = 500, 500
        EDGE_THRESHOLD_X = 30
        EDGE_THRESHOLD_Y = 30
        CORRECTION_RATIO = 0.20  # 向中心修正 20%

        if x < EDGE_THRESHOLD_X:
            x = int(x + (EDGE_THRESHOLD_X - x) * CORRECTION_RATIO)
            x = max(MARGIN, x)
        elif x > 1000 - EDGE_THRESHOLD_X:
            x = int(x - (x - (1000 - EDGE_THRESHOLD_X)) * CORRECTION_RATIO)
            x = min(1000 - MARGIN, x)

        if y < EDGE_THRESHOLD_Y:
            y = int(y + (EDGE_THRESHOLD_Y - y) * CORRECTION_RATIO)
            y = max(MARGIN, y)
        elif y > 1000 - EDGE_THRESHOLD_Y:
            y = int(y - (y - (1000 - EDGE_THRESHOLD_Y)) * CORRECTION_RATIO)
            y = min(1000 - MARGIN, y)
    ...
    def _build_current_prompt(self, input_data: AgentInput, loop_detected: bool) -> str:
        ...
        if self._action_history:
            last = self._action_history[-1]
            if last["action"] == ACTION_TYPE:
                parts.append(
                    "[重要] 你上一步已经输入了文字。现在请观察截图：\n"
                    "- 如果出现了「发送」「提交」「发布」「搜索」「确认」等按钮，请点击它\n"
                    "- 如果输入框中已显示你输入的文字，点击对应按钮完成任务\n"
                    "- 不要滚动页面，不要输入其他内容，不要点击无关按钮"
                )
            elif (len(self._action_history) >= 2
                  and self._action_history[-2]["action"] == ACTION_TYPE
                  and last["action"] == ACTION_CLICK
                  and last.get("page_changed", True)):
                # TYPE → CLICK（页面变化，说明点击了提交按钮且生效）
                parts.append(
                    "[重要] 你已经完成了文字输入并点击了按钮，页面已响应。"
                    "如果任务目标已经达成（如评论已发表、搜索已执行），请输出 COMPLETE:[]"
                )
...
## 任务完成判断（每一步都必须执行）
在「思考与决策」部分，必须明确回答：
- 任务目标是否已经达成？截图中的页面状态是否表明任务已完成？
- 常见完成信号：已成功打开目标页面、已找到目标内容、已成功提交订单等
- 如果任务已达成，输出 COMPLETE:[]

## 评论/评价类任务特别注意
如果当前任务是发表评论、写评价、好评等（涉及输入文字并提交）：
1. TYPE输入评论文字后，必须检查截图中是否出现「发送」「提交」「发布」按钮
2. 如果截图中可见发送/提交/发布按钮，必须先CLICK该按钮，再输出COMPLETE
3. 只有当截图中已经没有发送按钮（说明评论已自动提交），才可以直接COMPLETE
4. 绝不要在TYPE输入文字后立刻COMPLETE——除非截图中确认评论已成功提交
- 绝对不要在评论成功提交后继续执行多余操作（如再滚动或点其他按钮）""".format(instruction=instruction)
